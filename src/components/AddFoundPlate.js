...
function AddFoundPlate(props) {
  ...
  const handleFile = async (e) => {
    ...
    await fetch("http://api.izgubljene-tablice.com/picture", {
      method: "POST",
      body: formData,
    }).then((res) => res.json());
  };
  ...
  return (
    <div className="formWrapper">
      <div className="formContent">
        ...
        {inputStage === 1 ? (
          <div className="form">
            <p className="inputLabel">
              Broj tablice <span>*</span>
            </p>
            {warning ? (
              <p className="inputLabelWarning">
                Ova polja moraju da budu popunjena{" "}
              </p>
            ) : null}
            <input
              className={warning ? `formInput inputWarning` : `formInput`}
              type="text"
              placeholder="BG432CS"
              onChange={(e) => {
                e.preventDefault();
          ...
          <div>
            ...
            <Link to="/" className=" link">
              <p className="submitBTN formInput ">Upload</p>
            </Link>
          </div>
        )}
        ...
        <div className="tut">
          <p>
            <span>Broj Tablice</span> unesite broj bez razmaka ,- ,= ,/
          </p>
          <p>
            <span>Adresa</span> adresu koju unesete ce biti prikazana kao GPS
            pin na mapi
          </p>
        </div>
      </div>
...
const mapStateToProps = (state) => {
  return {
    ...
    total_found_plates: state.plateGlobal.total_found_plates,
  };
};
const mapDispatchToProps = (dispatch) => {
  return {
    createFoundPlate: (data) => dispatch(actionCreator.createFoundPlate(data)),
    plateChecker: (plateNumber) =>
      dispatch(actionCreator.plateChecker(plateNumber)),
    addGlobalFound: (newTotal) =>
      dispatch(actionCreator.addGlobalFound(newTotal)),
  };
};
